                      ...
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          <button
                            ...
                            className="inline-flex items-center rounded-full border border-slate-300 bg-white px-5 py-2 text-sm font-semibold text-slate-900 whitespace-nowrap"
                          >
                            {openFolders[pasta] ? 'Ocultar arquivos' : 'Ver arquivos'}
                          </button>
                          <a
                      ...
                      {openFolders[pasta] && (
                        <div className="mt-5 space-y-3 border-t border-slate-200 pt-4">
                          {pastaMateriais.map((material) => (
                            <div key={material.id} className="flex flex-col gap-4 rounded-[1.2rem] border border-slate-200 bg-white px-4 py-4 md:flex-row md:items-center md:justify-between">
                              <div className="flex min-w-0 items-center gap-4">
                                <MaterialThumb material={material} />
                                <div className="min-w-0">
                                  <p className="truncate text-base font-semibold text-slate-900">
                                    {material.titulo || material.caminhoRelativo}
                                  </p>
                                  <p className="mt-1 text-sm text-slate-600">
                                    {material.tipoArquivo || 'Documento'}
                                  </p>
                                  {material.caminhoRelativo && (
                              ...
                              <div className="flex flex-wrap gap-3">
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="inline-flex items-center rounded-full bg-hubBlueDeep px-4 py-2 text-sm font-semibold text-white"
                                >
                                  Abrir
                                </a>
                                <a
...
function MaterialThumb({ material }) {
  const isImage = material?.tipoArquivo === 'IMAGEM'

  if (isImage && material?.arquivoUrl) {
    return (
      <img
        src={material.arquivoUrl}
        alt={material.titulo || 'Material'}
  ...
  return (
    <div className="grid h-16 w-16 shrink-0 place-items-center rounded-xl border border-slate-200 bg-slate-50 text-slate-500">
      <span className="text-[11px] font-semibold uppercase tracking-[0.12em]">
        {material?.tipoArquivo || 'DOC'}
      </span>
    </div>
  )
}
